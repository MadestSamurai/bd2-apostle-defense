...
foreach($flavor in @('Portable','Lite')){
    ...
    if($identity.runtime -ne 'BD2ApostleDefense.Runtime6' -or $identity.version -ne $Version){throw 'Embedded identity differs from release'}
    ...
    $name="BD2ApostleDefense-$Version-$flavor-win-x64"
...
[ordered]@{version=$Version;runtime=$identity.runtime;compatibility='local-interface-adaptation';flavors=$flavors;languages=@('zh-CN','en-US');gameLibrariesBundled=$false;clientVersionLock=$false;runtimeVerification='source_implemented_pending_runtime'} | ConvertTo-Json -Depth 6 | Set-Content -LiteralPath (Join-Path $output 'release.json') -Encoding UTF8
