...
internal sealed class WindowLanguage : IDisposable
{
    ...
    private void Attach(DependencyObject node)
    {
        ...
        if(node is FrameworkElement element && (element.TemplatedParent!=null || element.Tag as string=="literal" || element.Name is "AccountText" or "SelectionTitle"))return;
        if(node is Button tile && tile.Tag is int)return;
