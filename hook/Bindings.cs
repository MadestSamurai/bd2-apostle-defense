...
namespace BD2ApostleDefense.Runtime
{
    internal static class Bindings
    {
        ...
        internal static MemberInfo Member(Type t,string name)
        {
            ...
            {
                var actual=ApostleClient.MemberNames.TryGetValue((next.IsGenericType?next.GetGenericTypeDefinition():next).FullName+"|"+name,out var mapped)?mapped:name;
        ...
        internal static Type Type(string role) {if(typeCache.TryGetValue(role,out var known))return known;return typeCache[role]=typeof(DefenseHUD).Assembly.GetType(ApostleClient.TypeNames.TryGetValue(role,out var name)?name:role,true);}
        internal static MemberInfo Api(string role) {if(apiCache.TryGetValue(role,out var known))return known;var entry=ApostleClient.Apis[role];var type=Type(entry[0]);return apiCache[role]=entry[2]=="method"?(MemberInfo)type.GetMethods(Flags).Single(m=>m.MetadataToken==int.Parse(entry[1])):Member(type,entry[1]);}
        ...
        internal static void ValidateCompiledClient()
        {
            ...
            if(typeof(DefenseHUD).Module.ModuleVersionId.ToString()!=ApostleClient.CompiledMvid)
                throw new InvalidOperationException("游戏运行中客户端文件发生变化，请正常重启游戏后重新连接。");
        }
        internal static int Validate() {foreach(var role in ApostleClient.Apis.Keys)Api(role);return ApostleClient.Apis.Count;}
