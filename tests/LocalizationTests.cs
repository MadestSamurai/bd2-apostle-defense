...
{
 public static string[] Sources(string ignored)
 {
  ...
  foreach(var dir in new[]{"desktop","core","shared","hook","compatibility","planner"})foreach(var file in Directory.GetFiles(Path.Combine(root!.FullName,dir),"*.cs"))
  {if(file.EndsWith("DemoPort.cs")||file.EndsWith("BoardSmoke.cs")||file.EndsWith("LanguageSmoke.cs"))continue;foreach(var node in CSharpSyntaxTree.ParseText(File.ReadAllText(file)).GetRoot().DescendantNodes())
   {if(node.Ancestors().OfType<MethodDeclarationSyntax>().Any(m=>m.Identifier.Text is "Smoke" or "BoardSmoke" or "LanguageSmoke"))continue;
    ...
    if(Regex.IsMatch(source,@"[\p{IsCJKUnifiedIdeographs}]"))result.Add(source);}}
  foreach(var file in Directory.GetFiles(Path.Combine(root!.FullName,"desktop"),"*.xaml"))foreach(var attr in XDocument.Load(file).Descendants().Attributes())if(Regex.IsMatch(attr.Value,@"[\p{IsCJKUnifiedIdeographs}]"))result.Add(attr.Value);
 ...
 public static int Run()
 {
  ...
  foreach(var dir in new[]{"desktop","core","shared","hook","compatibility","planner"})foreach(var file in Directory.GetFiles(Path.Combine(root!.FullName,dir),"*.cs"))
  {
   if(file.EndsWith("Smoke.cs")||file.EndsWith("DemoPort.cs"))continue;
